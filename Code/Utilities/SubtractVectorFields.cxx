...
int DoIt( int argc, char **argv )
{
  ...
  typename ReaderType::Pointer reader[2] = {ReaderType::New(),ReaderType::New() };
  ...
  // Software Guide : BeginCodeSnippet
  reader[0]->SetFileName( inputFilename[0] );
  reader[1]->SetFileName( inputFilename[1] );
  subtractImageFilter->SetInput1(reader[0]->GetOutput());
  subtractImageFilter->SetInput2(reader[1]->GetOutput());
  ...
  // Software Guide : BeginCodeSnippet
  writer->SetInput( subtractImageFilter->GetOutput() );
  // Software Guide : EndCodeSnippet
...
int main( int argc, char ** argv )
{
  ...
  unsigned int uiSourceImageDimension0 = CALATK::GetNonSingletonImageDimensionFromFile( inputFilename[0] );
  unsigned int uiSourceImageDimension1 = CALATK::GetNonSingletonImageDimensionFromFile( inputFilename[1] );

  if(uiSourceImageDimension0 != uiSourceImageDimension1)
  {
    std::cerr << "Image dimension missmatch" << std::endl;
    return EXIT_FAILURE;
  }

  switch ( uiSourceImageDimension0 )
  {
  case 2:
  ...
  case 3:
    ...
    break;
  default:
    std::cerr << "Unsupported image dimension = " << uiSourceImageDimension0 << std::endl;
  }
